...
class MPNNLayer(nn.Module):
    def __init__(self, num_hidden: int, num_edge: int, dropout: float = 0.1):
        super().__init__()
        ...
        # SwiGLU
        self.W_message = nn.Linear(num_hidden + num_edge, num_hidden * 2, bias=False)
        
        # Gating ساده
        self.W_gate = nn.Linear(num_hidden + num_edge, num_hidden, bias=False)
    ...
    def forward(self, h_V, h_E, mask_attend=None):
        ...
        # SwiGLU
        value, gate = self.W_message(h_EV).chunk(2, dim=-1)
        msg = value * F.silu(gate)
        
        # Gating با scaling ملایم (این تنها تغییری هست که نگه می‌داریم)
        message = msg * torch.sigmoid(self.W_gate(h_EV)) * 0.3   # 0.2~0.4 بهترین بازه است
        
        if mask_attend is not None:
            message = message * mask_attend.unsqueeze(-1)
            
        # Mean aggregation
        if mask_attend is not None:
            dh = message.sum(dim=-2) / mask_attend.sum(-1, keepdim=True).clamp(min=1.0)
        else:
            dh = message.mean(dim=-2)
        
        h_V = self.norm2(src + self.dropout(dh))
        return h_V
